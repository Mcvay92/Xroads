...
class Profile extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      isDialog:
        typeof localStorage.getItem("access_token") !== "string" ? false : true,
      error: ""
    };
  ...
  getProfilesData() {
    userService.getAllProfiles().then(
      response => {
        if (response.token == "invalid") {
          this.setState({
            isDialog: false,
  ...
  render() {
    ...
    if (this.state.profileData != null && this.state.profileData.length > 0) {
      profiles = this.state.profileData.map((item, key) => (
        <tr key={item._id}>
          <td>
            <Link to={`/profile/${item._id}`}>{item.team_name}</Link>
          </td>
          <td>{config.stages[item.stage]}</td>
          <td>{moment(new Date(item.start_date)).format("LL")}</td>
          <td>
            <Link className="mr-30" to={`/editProfile/${item._id}`}>
              <Edit />
            </Link>
          </td>
        </tr>
      ...
      <div className="col-12">
        <h4 className="mb-20">
          ...
          <span className="float-right">
            <Link to="/addProfile" className="btn btn-info">
              Add New Profile
            </Link>
          </span>
        </h4>
        ...
        {profileData &&
          profileData.length == 0 && (
            <div className="alert alert-info">No profile added yet.</div>
          )}
